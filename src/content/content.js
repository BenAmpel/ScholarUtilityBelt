(async () => {
  ...
  // The entire systematic-review workspace is lazy-loaded — see
  // src/content/review-workspace.js. Cached after first load so repeated
  // opens on the same page don't re-import.
  let reviewWorkspaceInstance = null;
  async function getReviewWorkspace() {
    if (!reviewWorkspaceInstance) {
      const { createReviewWorkspace } = await importModuleWithRetry("dist/content/review-workspace.js");
      reviewWorkspaceInstance = createReviewWorkspace({
        escapeHtml,
        csvEscape,
    ...
    return reviewWorkspaceInstance;
  }
  ...
  // Only the eager "match found" summary-card logic lives here — it runs on
  // every author-page visit (see ensureGenealogyMatchAsync's call site in
  // renderRightPanel). The tree traversal, edges dataset, and overlay
  // rendering only matter to the minority of users who open the Lineage
  // view, so that code is lazy-loaded from src/content/genealogy-lineage.js
  // — see getLineageOverlay() below and its call sites.
  ...
  // Tree traversal, edges dataset, and overlay rendering are lazy-loaded —
  // see src/content/genealogy-lineage.js. Cached after first load so
  // repeated opens on the same page don't re-import.
  let lineageOverlayInstance = null;
  async function getLineageOverlay() {
    if (!lineageOverlayInstance) {
      const { createLineageOverlay } = await importModuleWithRetry("dist/content/genealogy-lineage.js");
      lineageOverlayInstance = createLineageOverlay({
        escapeHtml,
        extractAuthorName,
    ...
    return lineageOverlayInstance;
  }
  async function openLineageOverlay() {
    const overlay = await getLineageOverlay();
    await overlay.open();
  }
  async function closeLineageOverlay() {
    const overlay = await getLineageOverlay();
    overlay.close();
  }
